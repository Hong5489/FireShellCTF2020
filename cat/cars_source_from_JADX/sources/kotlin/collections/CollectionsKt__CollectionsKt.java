...
class CollectionsKt__CollectionsKt extends CollectionsKt__CollectionsJVMKt {
    ...
    private static final <T> List<T> MutableList(int size, Function1<? super Integer, ? extends T> init) {
        ...
        for (int i = 0; i < size; i++) {
            list.add(init.invoke(Integer.valueOf(i)));
        }
        return list;
    }
    ...
    public static /* synthetic */ int binarySearchBy$default(List $this$binarySearchBy, Comparable key, int fromIndex, int toIndex, Function1 selector, int i, Object obj) {
        ...
        return CollectionsKt.binarySearch($this$binarySearchBy, fromIndex, toIndex, (Function1<? super T, Integer>) new CollectionsKt__CollectionsKt$binarySearchBy$1<Object,Integer>(selector, key));
    }
    ...
    public static final <T> int binarySearch(List<? extends T> $this$binarySearch, int fromIndex, int toIndex, Function1<? super T, Integer> comparison) {
        ...
        while (low <= high) {
            int mid = (low + high) >>> 1;
            int cmp = ((Number) comparison.invoke($this$binarySearch.get(mid))).intValue();
            if (cmp < 0) {
                low = mid + 1;
            } else if (cmp <= 0) {
                return mid;
