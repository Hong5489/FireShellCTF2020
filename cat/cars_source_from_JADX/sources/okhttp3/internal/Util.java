...
public final class Util {
    ...
    public static final Headers EMPTY_HEADERS = Headers.m36of(new String[0]);
    ...
    public static final Comparator<String> NATURAL_ORDER = $$Lambda$Util$TEfSBt3hRUlBSSARfPEHsJesTtE.INSTANCE;
    private static final Options UNICODE_BOMS = Options.m40of(ByteString.decodeHex("efbbbf"), ByteString.decodeHex("feff"), ByteString.decodeHex("fffe"), ByteString.decodeHex("0000ffff"), ByteString.decodeHex("ffff0000"));
    ...
    static /* synthetic */ Thread lambda$threadFactory$0(String name, boolean daemon, Runnable runnable) {
    ...
    public static Charset bomAwareCharset(BufferedSource source, Charset charset) throws IOException {
        int select = source.select(UNICODE_BOMS);
        if (select == -1) {
            return charset;
        }
        if (select == 0) {
    ...
    private static String inet6AddressToAscii(byte[] address) {
        ...
        while (i2 < address.length) {
            if (i2 == longestRunOffset) {
                ...
                result.writeHexadecimalUnsignedLong((long) (((address[i2] & UByte.MAX_VALUE) << 8) | (address[i2 + 1] & 255)));
                i2 += 2;
            }
        }
