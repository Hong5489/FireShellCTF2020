...
public final class Options extends AbstractList<ByteString> implements RandomAccess {
    ...
    /* renamed from: of */
    public static Options m40of(ByteString... byteStrings2) {
    ...
    private static void buildTrieRecursive(long nodeOffset, Buffer node, int byteStringOffset, List<ByteString> byteStrings2, int fromIndex, int toIndex, List<Integer> indexes) {
        ...
        if (fromIndex3 < i2) {
            ...
            ByteString from2 = (ByteString) byteStrings2.get(fromIndex);
            ByteString to = (ByteString) list.get(i2 - 1);
            if (i == from2.size()) {
                int prefixIndex3 = ((Integer) list2.get(fromIndex3)).intValue();
            ...
            if (from.getByte(i) != to.getByte(i)) {
                ...
                for (int i4 = fromIndex2 + 1; i4 < i2; i4++) {
                    if (((ByteString) list.get(i4 - 1)).getByte(i) != ((ByteString) list.get(i4)).getByte(i)) {
                        selectChoiceCount2++;
                    }
                }
                ...
                for (int i5 = fromIndex2; i5 < i2; i5++) {
                    byte rangeByte = ((ByteString) list.get(i5)).getByte(i);
                    if (i5 == fromIndex2 || rangeByte != ((ByteString) list.get(i5 - 1)).getByte(i)) {
                        buffer.writeInt((int) rangeByte & UByte.MAX_VALUE);
                    }
                }
                ...
                while (rangeStart < i2) {
                    ...
                    while (true) {
                        if (i6 >= i2) {
                            rangeEnd = rangeEnd3;
                            break;
                        } else if (rangeByte2 != ((ByteString) list.get(i6)).getByte(i)) {
                            rangeEnd = i6;
                            break;
                        } else {
                            i6++;
            ...
            for (int i11 = byteStringOffset; i11 < i + scanByteCount; i11++) {
                buffer.writeInt((int) from.getByte(i11) & UByte.MAX_VALUE);
            }
            if (fromIndex2 + 1 != i2) {
                int i12 = prefixIndex5;
            ...
            } else if (i + scanByteCount == ((ByteString) list.get(fromIndex2)).size()) {
                int i14 = prefixIndex5;
                buffer.writeInt(((Integer) indexes.get(fromIndex2)).intValue());
            } else {
                throw new AssertionError();
            }
        } else {
