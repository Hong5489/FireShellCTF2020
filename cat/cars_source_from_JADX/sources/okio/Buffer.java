...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
        ...
        if (head2 != null) {
            ...
            while (true) {
                ...
                if (triePos3 < 0) {
                    ...
                    i = c;
                    int selectChoiceCount = triePos3;
                    int pos3 = pos + 1;
                    int b = data[pos] & 255;
                    int selectLimit = triePos4 + selectChoiceCount;
                    while (triePos4 != selectLimit) {
                        if (b == trie[triePos4]) {
                            int nextStep = trie[triePos4 + selectChoiceCount];
                            if (pos3 == limit) {
                                s = s.next;
                                int pos4 = s.pos;
                                data = s.data;
    ...
    private ByteString digest(String algorithm) {
        try {
            ...
            return ByteString.m38of(messageDigest.digest());
    ...
    private ByteString hmac(String algorithm, ByteString key) {
        try {
            ...
            return ByteString.m38of(mac.doFinal());
        } catch (NoSuchAlgorithmException e) {
            throw new AssertionError();
