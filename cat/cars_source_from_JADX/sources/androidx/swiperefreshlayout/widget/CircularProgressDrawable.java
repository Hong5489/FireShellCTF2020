    ...
    private static class Ring {
        ...
        public void drawTriangle(Canvas c, float startAngle, float sweepAngle, RectF bounds) {
            if (this.mShowArrow) {
                ...
                float f = (float) this.mArrowWidth;
                float f2 = this.mArrowScale;
                path2.lineTo((f * f2) / 2.0f, ((float) this.mArrowHeight) * f2);
    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < SHRINK_OFFSET) {
                ...
                scaledTime = startTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - SHRINK_OFFSET) / SHRINK_OFFSET)) * 0.79f) + MIN_PROGRESS_ARC);
            }
